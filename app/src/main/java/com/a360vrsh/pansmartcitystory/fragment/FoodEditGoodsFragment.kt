...
import kotlinx.android.synthetic.main.fragment_food_edit_goods.*
 ...
 * 美食行业商品编辑添加
 */
class FoodEditGoodsFragment : BaseFragment(), OnResultCallbackListener<LocalMedia> {
    ...
    private var shelf_auto: String = "1"//1自动上架0手动上架
    ...
    override fun getLayoutID(): Int {
        return R.layout.fragment_food_edit_goods
    }
...
//                .selectionData(mAdapter?.data)// 是否传入已选图片
    ...
    override fun initListener() {
        ...
        edt_packing_cost.filters = EditTextUtil.inputTwoDecimal()
        ...
        ll_check.setOnClickListener {
            ...
            iv_uncheck.setImageResource(R.mipmap.icon_square_unchecked)
            shelf_auto = "1"
        }
        ll_uncheck.setOnClickListener {
            iv_check.setImageResource(R.mipmap.icon_square_unchecked)
            iv_uncheck.setImageResource(R.mipmap.icon_square_checked)
            shelf_auto = "0"
        }
    }
...
//            showToast("上传成功")
    ...
    private fun save() {
        ...
        val price_origin = edt_price_origin.text.toString().trim()
        val price_cost = edt_price_cost.text.toString().trim()
        val packing_cost = edt_packing_cost.text.toString().trim()
        ...
        if (title.isEmpty()) {
            ...
            showToast("请填写销售价格")
            return
        } else if (price_origin.isEmpty()) {
            showToast("请填写市场价格")
            return
        } else if (price_cost.isEmpty()) {
            showToast("请填写成本价格")
            return
        } else if (packing_cost.isEmpty()) {
            showToast("请填写包装费")
            return
        }
        ...
        paramMap["price_cost"] = price_cost
        paramMap["price_origin"] = price_origin
        paramMap["packing_cost"] = packing_cost
        paramMap["shelf_auto"] = shelf_auto
